...
@interface WLBaseInputView ()<UITextViewDelegate,UITextFieldDelegate>
...
///当前键盘类型
@property (nonatomic, assign) WLKeyboardType type;
///当前键盘索引, -1为z系统键盘
...
@implementation WLBaseInputView
...
- (void)keyboardValueChange:(NSNotification *)notification {
    ...
    if (beginFrame.origin.y < endFrame.origin.y) {
        if (self.keyboardIndex == -1) {
            ...
            //自定义键盘弹出
            [self WL_showKeyboardAtIndex:self.keyboardIndex duration:duration];
        }
    }
    else {
        //系统键盘弹出
        ...
        self.keyboardIndex = -1;
        self.type = WLKeyboardTypeSystem;
...
- (void)showSystemKeyboard {
    if (self.type != WLKeyboardTypeSystem) {
        self.type = WLKeyboardTypeSystem;
        if (self.inputView1) {
...
- (void)showKeyboardAtIndex:(NSInteger)index duration:(CGFloat)duration {
    ...
    if (self.type == WLKeyboardTypeSystem) {
        ...
        self.keyboardIndex = index;
        //直接弹出自定义键盘
        [self WL_showKeyboardAtIndex:index duration:duration];
    }
}

- (void)dismissKeyboard {
    if (self.type == WLKeyboardTypeIdle) return;
    if (self.type == WLKeyboardTypeSystem) {
        [self endEditing:YES];
    }
    else {
...
//直接弹出自定义键盘
- (void)WL_showKeyboardAtIndex:(NSInteger)index duration:(CGFloat)duration {
    //直接弹出自定义键盘
    self.type = WLKeyboardTypeOther;
    UIView *k;
    ...
    if (self.inputView1) {
        if (self.inputView1.text.length > 0) {
            ...
            if (length > 0) {
                ...
                NSString *emoticon = [[WLEmoticonManager manager] willDeleteEmoticon:subString];
                if ([[WLEmoticonManager manager].chs containsObject:emoticon]) {
