...
#import "WLBlankCell.h"
#import "WLEmojisCell.h"
#import "WLDeleteCell.h"
#import "WLEmoticonCell.h"
#import "WLHorizontalLayout.h"
#import "WLEmoticonManager.h"
#import "WLInputHelper.h"

#define key_rows  3
#define key_nums  7
@interface WLEmojisKeyboard ()<UICollectionViewDelegate,UICollectionViewDataSource>

@end

@implementation WLEmojisKeyboard {
    NSArray *_btns;
    NSArray *_emoticons;
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        //加载表情
        _emoticons = [WLEmoticonManager manager].emoticons;
        _emojisSection = _emoticons.count-1;
        
        CGFloat key_itemW = 45;
        CGFloat spcing = (320-key_itemW*key_nums)/(key_nums+1);
        WLHorizontalLayout *horLayout = [[WLHorizontalLayout alloc] initWithSpacing:spcing rows:key_rows nums:key_nums];
        
        CGRect rect = self.bounds;
        rect.size.height -= (40+[WLInputHelper helper].iPhoneXBottomH);
        ...
        [_collectionView registerClass:[WLBlankCell class] forCellWithReuseIdentifier:@"blank"];
        [_collectionView registerClass:[WLEmojisCell class] forCellWithReuseIdentifier:@"emojis"];
        [_collectionView registerClass:[WLDeleteCell class] forCellWithReuseIdentifier:@"delete"];
        [_collectionView registerClass:[WLEmoticonCell class] forCellWithReuseIdentifier:@"emoticon"];
        [self addSubview:_collectionView];
        
        UIColor *themeColor = [UIColor colorWithRed:34/255. green:207/255. blue:172/255. alpha:1];
        UIView *toolView = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_collectionView.frame), frame.size.width, 40+[WLInputHelper helper].iPhoneXBottomH)];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([self isDelete:indexPath.item]) {
        WLDeleteCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"delete" forIndexPath:indexPath];
        return cell;
    }
    else {
        ...
        if (index < emojis.count) {
            if (indexPath.section == _emojisSection) {
                ...
                WLEmojisCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"emojis" forIndexPath:indexPath];
                [cell setConfig:text];
                return cell;
            }
            else {
                ...
                WLEmoticonCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"emoticon" forIndexPath:indexPath];
                [cell setConfig:[dic objectForKey:@"png"]];
                return cell;
            }
        }
        else {
            WLBlankCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"blank" forIndexPath:indexPath];
            return cell;
        }
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    NSInteger index = scrollView.contentOffset.x/[WLInputHelper helper].screenW;
...
- (void)toolBtnClick:(UIButton *)btn {
    ...
    [_collectionView setContentOffset:CGPointMake([WLInputHelper helper].screenW*index, 0) animated:NO];
}
