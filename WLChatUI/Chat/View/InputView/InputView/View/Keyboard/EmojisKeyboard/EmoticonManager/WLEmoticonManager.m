...
#import "WLInputBase64.h"

@interface WLEmoticonManager ()
...
@implementation WLEmoticonManager

+ (instancetype)manager {
    static WLEmoticonManager *m;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        m = [[WLEmoticonManager alloc] init];
    });
    return m;
}
...
- (void)loadEmoticons {
    ...
    //默认表情
    NSString *path1 = [[NSBundle mainBundle] pathForResource:@"WLEmoticon1" ofType:@"plist"];
    NSArray *array1 = [[NSArray alloc] initWithContentsOfFile:path1];
    [emoticons addObject:array1];
    
    //浪小花
    NSString *path2 = [[NSBundle mainBundle] pathForResource:@"WLEmoticon2" ofType:@"plist"];
    NSArray *array2 = [[NSArray alloc] initWithContentsOfFile:path2];
    [emoticons addObject:array2];
    //emojis
    NSString *emojisPath = [[NSBundle mainBundle] pathForResource:@"WLEmojis" ofType:@"plist"];
...
- (NSMutableAttributedString *)attributedString:(NSString *)aString {
    ...
    if (!error) {
        ...
        for (NSTextCheckingResult *match in matchs) {
            ...
            UIImage *image = [WLInputHelper emoticonImageNamed:imageName];
            if (image) {
