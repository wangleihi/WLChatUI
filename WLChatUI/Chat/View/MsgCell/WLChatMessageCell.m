...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        _retryBtn = [WLChatBtn chatButtonWithType:WLChatButtonTypeRetry];
        [_retryBtn setImage:[WLInputHelper otherImageNamed:@"WL_chat_retry"] forState:UIControlStateNormal];
...
- (void)setConfig:(WLChatMessageModel *)model isShowName:(BOOL)isShowName {
    ...
    if (model.isSender) {
        ...
        _bubbleImageView.image = [WLChatHelper senderBubble];
        
        //消息内容
        CGRect rect = _bubbleImageView.frame;
        if (model.msgType == WLMessageTypeText) {
            rect.origin.x += 5;
            rect.size.width -= 17;
        }
        _contentRect = rect;
        ...
        if (model.sendType == WLMessageSendTypeWaiting) {
            _activityView.hidden = NO;
            [_activityView startAnimating];
            
            _retryBtn.hidden = YES;
        }
        else if (model.sendType == WLMessageSendTypeSuccess) {
        ...
        _bubbleImageView.image = [WLChatHelper receiverBubble];
