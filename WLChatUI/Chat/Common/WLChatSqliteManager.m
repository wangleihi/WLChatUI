...
@interface WLChatSqliteManager () {
    NSString *_dataBasePath;
    sqlite3 *_sql3;
}
@end

@implementation WLChatSqliteManager
...
+ (instancetype)defaultManager{
    ...
    static WLChatSqliteManager *manager;
    dispatch_once(&onceToken, ^{
        manager = [[WLChatSqliteManager alloc] init];
    });
    return manager;
}
...
- (NSArray *)allPropertyNameInClass:(Class)cls{
    if ([cls isSubclassOfClass:[WLChatBaseModel class]]) {
...
- (NSDictionary *)DictionaryFromModel:(id)model{
    if ([model isKindOfClass:[WLChatBaseModel class]]) {
