...
#import "NSDateFormatter+WLChat.h"

@interface WLChatHelper ()
...
@implementation WLChatHelper

+ (instancetype)helper {
    static WLChatHelper *helper;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        helper = [[WLChatHelper alloc] init];
    });
    return helper;
}
...
+ (UIImage *)senderBubble {
    return [WLChatHelper helper].senderBubbleImage;
}

+ (UIImage *)receiverBubble {
    return [WLChatHelper helper].receiverBubbleImage;
}
...
+ (UIImage *)getImageFromCacheWithUrl:(NSString *)url {
    //1、从内存存获取
    WLChatHelper *helper = [WLChatHelper helper];
...
- (UIImage *)senderBubbleImage {
    if (_senderBubbleImage == nil) {
        UIImage *image = [WLInputHelper otherImageNamed:@"WL_chat_bj2"];
...
- (UIImage *)receiverBubbleImage {
    if (_receiverBubbleImage == nil) {
        UIImage *image = [WLInputHelper otherImageNamed:@"WL_chat_bj1"];
        CGSize size = image.size;
