...
@implementation WLChatMessageModel {
...
- (instancetype)init {
    ...
    if (self) {
        self.modelH = -1;
        self.modelW = -1;
        self.mid = [NSString stringWithFormat:@"%@",@([WLChatHelper nowTimestamp])];
    }
    return self;
}
...
- (void)cacheModelSize {
    if (self.modelH == -1 || self.modelW == -1) {
        if (self.msgType == WLMessageTypeSystem) {
            self.modelH = 20;
            self.modelW = CHAT_SCREEN_WIDTH;
        }
        else if (self.msgType == WLMessageTypeText) {
        ...
        else if (self.msgType == WLMessageTypeImage) {
            [self handleImageSize];
            self.modelH = self.imgH;
            self.modelW = self.imgW;
        }
        else if (self.msgType == WLMessageTypeVoice) {
            CGFloat minW = 60;
            CGFloat dw = 5.2;
        ...
        else if (self.msgType == WLMessageTypeVideo) {
            [self handleImageSize];
            self.modelH = self.imgH;
            self.modelW = self.imgW;
        }
...
- (NSAttributedString *)attributedString {
    if (_attStr == nil) {
        ...
        NSMutableAttributedString *att = [[WLEmoticonManager manager] attributedString:self.message];
