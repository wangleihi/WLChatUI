...
- (void)loadUser {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        self.users = [[WLChatDBManager DBManager] users];
...
- (void)rightItemClick {
    NSString *uid = @"00002";
    WLChatUserModel *lastModel = self.users.lastObject;
    if (lastModel) {
        uid = [NSString stringWithFormat:@"%05ld",(long)(lastModel.uid.integerValue+1)];
    }
    
    WLChatUserModel *model = [[WLChatUserModel alloc] init];
    model.uid = uid;
    ...
    [self.tableView reloadData];
    //加入数据库
    [[WLChatDBManager DBManager] insertUserModel:model];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row < self.users.count) {
        WLChatUserModel *model = [self.users objectAtIndex:indexPath.row];
        
        WLChatViewController *chatVC = [[WLChatViewController alloc] initWithUser:model];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    WLUserTableViewCell *cell = (WLUserTableViewCell *)[tableView dequeueReusableCellWithIdentifier:@"cell"];
    if (cell == nil) {
        cell = [[WLUserTableViewCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:@"cell"];
    }
    if (indexPath.row < self.users.count) {
        WLChatUserModel *model = [self.users objectAtIndex:indexPath.row];
...
- (NSArray<UITableViewRowAction *> *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row < self.users.count) {
        ...
        UITableViewRowAction *deleteAction = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDefault title:@"删除" handler:^(UITableViewRowAction * _Nonnull action, NSIndexPath * _Nonnull indexPath) {
            ...
            [self.tableView reloadData];
            [[WLChatDBManager DBManager] deleteUserModel:model.uid];
